...
def retrieve_job_by_id(id:int, db:Session=Depends(get_db)):
    ...
    if not job:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND,detail=f"Job with if {id} does not exist")
    return job
...
def delete_job(id:int,db:Session=Depends(get_db)):
    ...
    if not message:
        raise HTTPException(status=status.HTTP_404_NOT_FOUND,detail=f"Job with id {id} does not exist")
